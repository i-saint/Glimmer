...
void GfxContextDXR::prepare(RenderDataDXR& rd)
{
    ...
    if (!rd.desc_heap) {
        ...
        lptSetName(rd.desc_heap, rd.name + " Desc Heap");
    ...
    if (!rd.scene_data) {
        ...
        lptSetName(rd.scene_data, rd.name + " Scene Data");
    ...
    lptTimestampInitialize(rd.timestamp, m_device);
    lptTimestampReset(rd.timestamp);
    lptTimestampSetEnable(rd.timestamp, GetGlobals().hasDebugFlag(DebugFlag::Timestamp));
...
#ifdef lptEnableRenderTargetValidation
    if (rd.render_target) {
...
#endif // lptEnableRenderTargetValidation
}
...
void GfxContextDXR::setMeshes(RenderDataDXR& rd, std::vector<MeshInstanceDataPtr>& instances)
{
    ...
    for (auto& inst : instances) {
        ...
        if (!mesh_dxr->vertex_buffer) {
            ...
            if (!mesh_dxr->vertex_buffer->resource) {
                lptDebugPrint("GfxContextDXR::setMeshes(): failed to translate vertex buffer\n");
                continue;
            }
            if ((mesh_dxr->vertex_buffer->size / mesh->vertex_count) % 4 != 0) {
                lptDebugPrint("GfxContextDXR::setMeshes(): unrecognizable vertex format\n");
                continue;
            }
            lptSetName(mesh_dxr->vertex_buffer->resource, mesh->name + " VB");
        ...
        if (!mesh_dxr->index_buffer) {
            ...
            if (!mesh_dxr->index_buffer->resource) {
                lptDebugPrint("GfxContextDXR::setMeshes(): failed to translate index buffer\n");
                continue;
            }
            lptSetName(mesh_dxr->index_buffer->resource, mesh->name + " IB");
    ...
    lptTimestampQuery(rd.timestamp, cl_blas, "Building BLAS begin");
        ...
        if (gpu_skinning && inst_dxr.deformed_vertices) {
            if (!inst_dxr.blas_deformed || inst.isUpdated(UpdateFlag::Deform)) {
                ...
                if (!inst_dxr.blas_deformed) {
                    ...
                    lptSetName(inst_dxr.blas_scratch, inst.name + " BLAS Scratch");
                    lptSetName(inst_dxr.blas_deformed, inst.name + " BLAS");
                }
            ...
            if (!mesh_dxr.blas || mesh_dxr.vertex_buffer->is_updated) {
                ...
                if (!mesh_dxr.blas) {
                    ...
                    lptSetName(mesh_dxr.blas_scratch, mesh.name + " BLAS Scratch");
                    lptSetName(mesh_dxr.blas, mesh.name + " BLAS");
                }
    ...
    lptTimestampQuery(rd.timestamp, cl_blas, "Building BLAS end");
    cl_blas->Close();
    ...
    lptTimestampQuery(rd.timestamp, cl_tlas, "Building TLAS begin");
    if (needs_build_tlas) {
        ...
        ReuseOrExpandBuffer(td.instance_desc, sizeof(D3D12_RAYTRACING_INSTANCE_DESC), instance_count, 4096, [this, &rd](size_t size) {
            ...
            lptSetName(ret, rd.name + " Instance Desk");
            return ret;
        });
        ...
        // create TLAS
        {
            ...
            ReuseOrExpandBuffer(td.scratch, 1, info.ScratchDataSizeInBytes, 1024 * 64, [this, &rd](size_t size) {
                ...
                lptSetName(ret, rd.name + " TLAS Scratch");
                return ret;
            });
    ...
    lptTimestampQuery(rd.timestamp, cl_tlas, "Building TLAS end");
    cl_tlas->Close();
    ...
    if (needs_build_tlas) {
        ...
        bool expanded = ReuseOrExpandBuffer(rd.instance_data, stride, instance_count, 4096, [this, &rd](size_t size) {
            auto ret = createBuffer(size, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, kUploadHeapProps);
            lptSetName(ret, rd.name + " Instance Data");
            return ret;
        });
        if (expanded) {
...
void GfxContextDXR::flush(RenderDataDXR& rd)
{
    ...
    lptTimestampQuery(rd.timestamp, cl_rays, "DispatchRays begin");
    ...
    lptTimestampQuery(rd.timestamp, cl_rays, "DispatchRays end");
    lptTimestampResolve(rd.timestamp, cl_rays);
...
bool GfxContextDXR::finish(RenderDataDXR& rd)
{
    ...
    if (rd.fv_rays != 0) {
        ...
        lptTimestampUpdateLog(rd.timestamp, m_cmd_queue_direct);
...
void GfxContextDXR::frameEnd()
{
    ...
    auto erase_unused_records = [](auto& records, const char *message) {
        ...
        if (num_erased > 0) {
            lptDebugPrint(message, num_erased);
        }
    };
